<?php
include_once("ok.php");
$sqlQuery = "SELECT * FROM tbl_sevac ORDER BY año desc";
        ...
        <div class="row">
          <div class="col-lg-12">
              <div class="banner-heading">
                <h1 class="banner-title">Cuentas claras</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb justify-content-center">
                      <li class="breadcrumb-item"><a href="#">Toda la información financiera</a></li>
                    </ol>
                </nav>
              </div>
          </div><!-- Col end -->
...
<div class="row">
      <div class="col-md-12">
          <div class="form-group">
            ...
            <div class="form-group">
              <select class="combobox form-control " name="ar" id="ar" required>
                ...
                <?php 
                    require_once('ok.php');
                    $sql = "SELECT id, año FROM tbl_años";
                    $query = $conn -> query ($sql);
                    while($valores = mysqli_fetch_array($query)){
                        echo "<option value='".$valores['año']."'>".$valores['año']."</option>";
                    }
                    ?>
              </select>
            </div>
          ...
          <div class="form-group">
            ...
            <div class="form-group">
              <select class="combobox form-control" name="ar" id="trimestre" required>
                <option value="" selected="selected">Selecciona Trimestre</option>
                <?php 
                    require_once('ok.php');
                    $sql = "SELECT trimestre FROM tbl_sevac";
                    $query = $conn -> query ($sql);
                    while($valores = mysqli_fetch_array($query)){
                        echo "<option value='".$valores['trimestre']."'>".$valores['trimestre']."</option>";
                    }
                    ?>
              </select>
            </div>
            ...
            <table id="editableTable" class="table table-striped table-bordered" style="margin-bottom: 0">
              <thead >
                <tr>
                  <th>Documento</th>
                  <th>Año</th>
              ...
              <tbody id="fbody" class="contenidobusqueda">
                <?php 
                require_once('ok.php');
                $kuery =mysqli_query($conn,"SELECT * FROM tbl_sevac ORDER BY año desc");
                
                while( $developer = mysqli_fetch_assoc($kuery) ) { ?>
                   <tr id="<?php echo $developer ['id']; ?>">
                   <td><?php echo '<img src="images/file_extension_pdf.png" ALIGN=LEFT><a href="'.$developer['doc'].'">'.$developer['namea'].'</a>'; ?></td>
                   <td><?php echo $developer ['año']; ?></td> 				   				   				  
                   <td><?php echo $developer ['trimestre']; ?></td> 				   				   				  
                   </tr>
                <?php } ?>
              </tbody>
            </table>
